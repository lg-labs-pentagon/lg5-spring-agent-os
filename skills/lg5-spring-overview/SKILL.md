...
Framework artifacts are versioned `1.0.0-alpha.<short-git-sha>` (e.g. `1.0.0-alpha.cbb6783`). Consumers pin the SHA explicitly. To bump: pick a newer SHA from `git -C /tmp/lg5-study/lg5-spring log --oneline -20` and run `make publish-local` in the framework repo.
